...
dump_path = '/workspace/data/all_labels_and_aliases.wdump-4526.nt.gz'
...
with bz2.BZ2File(dump_path, "r") as dump:
    ...
    with psycopg.connect(postgres_connection, autocommit=False) as conn:
        with conn.cursor() as cur:
            if force:
                print('deleting from entitylabelswd...')
                cur.execute('DELETE FROM entitylabelswd;')
            conn.commit()
                ...
                for bline in dump:
                    ...
                    if 'rdf-schema#label' in line: # not sure if this is faster
                        ...
                        if len(chunk) >= chunk_size:
                            with cur.copy("COPY entitylabelswd (subject, object) FROM STDIN") as copy:
                                for item in chunk:
                                    copy.write_row(item)
                            conn.commit()
                            chunk = []
